...
Personal, single-user, Dockerized, web-based music player. Optimized for simplicity. Treat this as a high-level, vague reference.

## Architecture decisions (locked in)

- One app container: backend serves API + streams audio + serves built frontend static files (no separate frontend server).
- Backend: FastAPI (Python) — decided, `backend/` scaffolded with uv + FastAPI.
- Frontend: SvelteKit — decided, `frontend/` scaffolded via `sv create` (TS, prettier, eslint, vitest, tailwindcss).
- DB: SQLite (single file, no Postgres).
- Storage: music folder mounted read-only; separate volume for SQLite + cache data.
- Metadata: `mutagen` (Python) for ID3/tag reading.
...
- [x] Decide backend language: FastAPI vs Express → FastAPI
- [x] Init backend project structure (`backend/`, uv-managed, hello-world + `/health` route)
- [x] Init frontend project (SvelteKit) — mockups only, no real logic code yet
...
- [ ] Synced lyrics (line-level, Spotify-style auto-scroll/highlight) — source from `.lrc` sidecar files and/or LRCLIB API lookup, cache in SQLite; word-level karaoke sync out of scope (needs proprietary timing data)
  - [ ] In-app "tap to sync" `.lrc` editor (play track, hit key per line to timestamp, save/export `.lrc`) for tracks with no synced lyrics available
